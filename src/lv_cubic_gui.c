...
void display_init(void)
{
  ...
  lv_style_set_text_font(&label_style2, LV_STATE_DEFAULT, &lv_font_montserrat_24);
  ...
  instru_label = lv_label_create(scr[3],NULL);
}
...
{
  char path[25] = {0};
  sprintf(path, "S:/time/time.bin");
  // lv_img_set_src(time_image, &huoj);
  lv_img_set_src(time_image, path);
...
void display_setting(const char *info, const char *ap_ip, const char *domain, const char *title)
{
  ...
  // lv_obj_add_style(instru_label, LV_LABEL_PART_MAIN, &label_style5);
  // lv_label_set_text(instru_label, "Type the domain\n in the explorer\n to access.");
  // lv_obj_align(instru_label, NULL, LV_ALIGN_IN_BOTTOM_LEFT, 5, -110);
